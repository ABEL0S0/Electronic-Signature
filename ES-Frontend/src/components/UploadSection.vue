...
<template>
  <div class="grid lg:grid-cols-2 gap-8">
    ...
    <div class="bg-white/90 rounded-xl shadow p-8 flex flex-col">
      ...
      <div v-if="!documentFile" class="flex flex-col items-center justify-center border-2 border-dashed border-emerald-200 rounded-xl py-10 mb-4 cursor-pointer hover:border-emerald-400 transition-all" @click="triggerDocumentInput">
        ...
        <button type="button" class="mt-2 px-4 py-2 bg-white border border-emerald-200 rounded text-emerald-700 font-semibold hover:bg-emerald-50" @click.stop="triggerDocumentInput">Seleccionar Archivo PDF</button>
        <span class="text-xs text-slate-400 mt-2">Máximo 10MB · Solo archivos PDF</span>
      </div>
      <div v-else class="border-2 border-emerald-200 rounded-xl p-4 mb-4 bg-emerald-50">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <p class="font-medium text-slate-900">{{ documentFile.name }}</p>
              <p class="text-sm text-slate-600">{{ (documentFile.size / 1024 / 1024).toFixed(2) }} MB</p>
            </div>
          </div>
          <button @click="documentFile = null" class="text-red-500 hover:text-red-700">
    ...
    <div class="bg-white/90 rounded-xl shadow p-8 flex flex-col">
      ...
      <div v-if="!certificateFile" class="flex flex-col items-center justify-center border-2 border-dashed border-teal-200 rounded-xl py-10 mb-4 cursor-pointer hover:border-teal-400 transition-all" @click="triggerCertificateInput">
        ...
        <button type="button" class="mt-2 px-4 py-2 bg-white border border-teal-200 rounded text-teal-700 font-semibold hover:bg-teal-50" @click.stop="triggerCertificateInput">Seleccionar Certificado</button>
        <span class="text-xs text-slate-400 mt-2">Formatos: .p12, .pfx</span>
      </div>
      <div v-else class="border-2 border-teal-200 rounded-xl p-4 mb-4 bg-teal-50">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            <div class="w-10 h-10 bg-teal-100 rounded-lg flex items-center justify-center">
              <svg class="w-5 h-5 text-teal-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 22s8-4 8-10V5.236A2.236 2.236 0 0017.764 3H6.236A2.236 2.236 0 004 5.236V12c0 6 8 10 8 10z"/></svg>
            </div>
            <div>
              <p class="font-medium text-slate-900">{{ certificateFile.name }}</p>
              <p class="text-sm text-slate-600">{{ (certificateFile.size / 1024 / 1024).toFixed(2) }} MB</p>
            </div>
          </div>
          <button @click="certificateFile = null" class="text-red-500 hover:text-red-700">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
      </div>
